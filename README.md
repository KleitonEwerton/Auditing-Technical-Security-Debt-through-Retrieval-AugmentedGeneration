...
> 
> **Nota sobre a Variação de Métricas com o Novo Modelo (Forma B):** A transição forçada para o modelo substituto evidenciou uma alteração no perfil de inferência. O modelo original apresentava maior propensão à adivinhação em casos ambíguos. O modelo atual demonstra um comportamento estritamente conservador (alta precisão, menor revocação). Em casos onde o código apresenta defesas superficiais (e.g., uso de SecureRandom com SHA1PRNG em instâncias avaliadas pelo OWASP Benchmark como CWE-330), o modelo substituto opta por classificar a amostra como segura (`"cwe_id": "None"`, `"stride": "None"`), aumentando a taxa de Falsos Negativos. O framework registrou fielmente essa mudança de comportamento sem qualquer alteração nos prompts ou na metodologia, validando a capacidade da arquitetura de isolar e avaliar diferentes motores generativos.
